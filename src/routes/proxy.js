...
// Used by SillyTavern, JanitorAI, and any OpenAI-compatible client
router.post('/v1/chat/completions', requireProxyKey, async (req, res) => {
  ...
  try {
    ...
    // Get master key to determine provider
    const mk = db.prepare('SELECT * FROM master_keys WHERE id = ? AND is_active = 1').get(pk.master_key_id);
    if (!mk) {
      logData.status = 'error';
      logData.error = 'Master key unavailable';
      logRequest(logData);
      return res.status(503).json({ error: 'Provider key unavailable' });
    }
    ...
    if (pk.csam_scan_enabled) {
      ...
      if (scan.flagged) {
        logData.status = 'blocked_csam';
        logData.csamFlagged = true;
        logData.csamScore = scan.score;
        logData.latency = Date.now() - start;
        logRequest(logData);
        return res.status(451).json({ error: 'Content blocked by safety filter' });
      }
    }

    // Context lock check
    if (pk.context_lock_enabled) {
      const allowedContexts = JSON.parse(pk.allowed_contexts || '["general"]');
      const ctxCheck = checkContextLock(messages, allowedContexts);
      if (!ctxCheck.allowed) {
        logData.status = 'context_locked';
        logData.error = ctxCheck.reason;
        logData.latency = Date.now() - start;
        logRequest(logData);
        return res.status(403).json({ error: ctxCheck.reason });
      }
    }
    ...
    logData.status = 'success';
    logData.promptTokens = result.tokens?.prompt || 0;
    logData.completionTokens = result.tokens?.completion || 0;
    logData.latency = result.latency;
    logRequest(logData);

    res.json(result.data);
  } catch (err) {
    logger.error('Proxy request failed', { error: err.message, userId: pk.user_id });
    logData.status = 'error';
    logData.error = err.message;
    logData.latency = Date.now() - start;
    logRequest(logData);
...
// List available models from the master key
router.get('/v1/models', requireProxyKey, (req, res) => {
  ...
  let models = [];
  try {
    models = JSON.parse(mk.models || '[]');
  } catch {}

  // Default models per provider if none set
  if (!models.length) {
    const defaults = {
  ...
  res.json({
    object: 'list',
    data: models.map(id => ({ id, object: 'model', created: 1700000000, owned_by: mk.provider })),
  });
});
